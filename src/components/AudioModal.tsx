...
import { CheckIcon, MicIcon, PauseIcon, PlayIcon, RefreshCcwIcon, SquareIcon, Trash2Icon, XIcon } from 'lucide-react-native';
...
export function AudioModal({ onClose, open }: IAudioModalProps) {
    ...
    const isAudioPlaying = isRecording || playerStatus.playing;

    useEffect(() => {
        (async () => {
            const status = await AudioModule.requestRecordingPermissionsAsync();
            if (!status.granted) {
                Alert.alert('A permissão para gravação foi negada.');
            }
                            ...
                            <View className="size-[265px] border border-gray-700/10 rounded-full items-center justify-center">
                                <View
                                    className={cn(
                                        ...
                                        isAudioPlaying && 'border-lime-600/50',
                                    )}
                                >
                                    <View
                                        className={cn(
                                            'size-[179px] bg-gray-700/10 rounded-full',
                                            isAudioPlaying && 'bg-lime-600/10',
                                        )}
                                    />
                                </View>
                            </View>
                        ...
                        {audioUri && (
                            <View className="p-5 pt-6 items-center pb-20 flex-row justify-center gap-8">
                                ...
                                {!playerStatus.playing && (
                                    <Button size="icon" color="dark" onPress={() => {
                                        ...
                                        {playerStatus.didJustFinish
                                            ? <RefreshCcwIcon size={20} color={colors.lime[600]} />
